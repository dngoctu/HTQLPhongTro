...
<h1 class="text-center">Quản lí người thuê</h1>

<c:url value="/nguoithue/nguoithuedetails" var="action" />
<form:form method="post" action="${action}" modelAttribute="nguoithue" enctype="multipart/form-data">
    ...
    <div class="form-floating mb-3 mt-3">
        <form:select class="form-select" id="idtaiKhoan"  path="idtaiKhoan.id">
            <c:forEach items="${taiKhoan}" var="tk">
                <c:choose>
                    <c:when test="${tk.id==nguoithue.idtaiKhoan.id}">
    ...
    <div class="form-floating">
        <button class="btn btn-info mt-1" type="submit">
            <c:choose>
                <c:when test="${nguoithue.id > 0}"> Cập nhật </c:when>
