<%-- 
    Document   : stats
    Created on : May 11, 2024, 8:40:19 AM
    Author     : DikamonTu
--%>
...
<div class="row">
    <div class="col-md-5 col-12">
        <table class="table table-striped">
            ...
            <tr>
                <td>Tổng số người thuê:</td>
                <td>${statsByNguoiThue}</td>
            </tr>
            <tr>
                <td>Tổng số chủ trọ:</td>
                <td>${statsByChuTro}</td>
            </tr>
        </table>
    </div>
...
<script>
    let labels = [];
    let data = [];
    <c:forEach items="${statsNguoiThueByPeriod}" var="p">
    labels.push(${p[0]});
    data.push(${p[1]});
    </c:forEach>
    ...
    let label2 = [];
    let data2 = [];
    <c:forEach items="${statsChuTroByPeriod}" var="p">
    label2.push(${p[0]});
    data2.push(${p[1]});
    </c:forEach>
    ...
    window.onload = function () {
        ...
        drawChartRevenue(ctx1, labels, data, "Người thuê");

        let ctx2 = document.getElementById("myChart2");
        drawChartRevenue(ctx2, label2, data2, "Chủ trọ");
